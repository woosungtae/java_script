		...
		<tbody align=center>
			...
			<tr>
				<c:forEach var="item" items="${myOrderList }">
					...
					<td><h2><fmt:formatNumber value="${item.goods_sales_price*0.9}" pattern="#,###" /> 원 (10% 할인)</h2></td>
					<c:forEach var="item2" items="${cartMap.myGoodsList}">
					<c:choose>
					<c:when test="${item2.goods_id == item.goods_id}">
					<td><h2><fmt:formatNumber value="${item2.goods_delivery_price}" pattern="#,###" /> 원</h2><c:set var="total_delivery_price" value="${total_delivery_price + item2.goods_delivery_price }"/></td>
					</c:when>
					</c:choose>
					</c:forEach>
					
					<td><h2><fmt:formatNumber value="${1500 *item.order_goods_qty}" pattern="#,###" />원</h2></td>
					<td>
					  <h2><fmt:formatNumber value="${item.goods_sales_price*0.9 * item.order_goods_qty}" pattern="#,###" />원</h2>
	...
	<table width=80% class="list_view" style="background: #ccffff">
		<tbody>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_totalPrice"><fmt:formatNumber value="${total_order_price}" pattern="#,###" /> 원</p> <input
					id="h_totalPrice" type="hidden" value="${total_order_price}" />
				</td>
				<td><IMG width="25" alt=""
					src="${pageContext.request.contextPath}/resources/image/plus.jpg"></td>
				<td>
					<p id="p_totalDelivery"><fmt:formatNumber value="${total_delivery_price}" pattern="#,###" /> 원</p> <input
				...
				<td>
					<p id="p_totalSalesPrice"><fmt:formatNumber value="${final_total_order_price*0.1}" pattern="#,###" /> 원</p> 
					<input id="h_total_sales_price" type="hidden" value="${final_total_order_price*0.1}" />
				</td>
				<td><img width="25" alt="" src="${pageContext.request.contextPath}/resources/image/equal.jpg"></td>
				<td>
					<p id="p_final_totalPrice">
						<font size="15"><fmt:formatNumber value="${final_total_order_price*0.9 +total_delivery_price}" pattern="#,###" /> 원 </font>
					</p> <input id="h_final_total_Price" type="hidden" value="${final_total_order_price*0.9 +total_delivery_price}" />
				</td>
			</tr>
		</tbody>
	</table>
